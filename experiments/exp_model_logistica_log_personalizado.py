...
if __name__ == "__main__":
    ...
    for model_name in models:
        ...
        with mlflow.start_run(run_name=f"novo_modelo_{model_name}_v3"):
            ...
            model_loader = build_model_loader(model_name)
            model = model_loader(X=df.loc[train_index][features], y=df.loc[train_index][label])
            print("Modelo treinado com sucesso!")
            
            # Salvando o modelo com o log_model específico
            if model_name == "logistic":
                mlflow.sklearn.log_model(model, artifact_path="models/logistic")
            elif model_name == "lgbm":
                mlflow.lightgbm.log_model(model, artifact_path="models/lgbm")
            elif model_name == "xgb":
                mlflow.xgboost.log_model(model, artifact_path="models/xgb")
